...
class starter {
		...
		*/
		while(true){
			...
			if (choice.equals("1")){
				...
				arr = fillArray(textFile2);		
			}
			else if (choice.equals("6")){
				System.out.println("Search for what word?");
				String test = sc.nextLine();
				System.out.println("The amount of time it appears is " + checksArray(arr, test));
			}
	...
	*/
	public static int checksArray(String[] arr, String check){
		...
		return counter;	// Dummy value, delete when needed.
	}
	...
	*/
	public static String mostCommon(String[] arr){
		...
		int highestNumberOfOccurances = 0;
		...
		if (counter > highestNumberOfOccurances){
			highestNumberOfOccurances = counter;
			mostCommonWord = word;
		}
