			...
			for (int i = 0; i < board[0].length; i++){
				for (int j = 0; j < board.length; j++){
					...
					if (board[j][i] == "A"){
						...
						if(board[j - 1][i] == "A" && board[j - 2][i] == "A" && board[j - 3][i] == "A"){
							return 1;
						}
					}
				}
			...
			for (int i = 0; i < board[0].length; i++){
				for (int j = 0; j < board.length; j++){
					...
					if (board[j][i] == "B"){
						...
						// int startingColum = j;
						if(board[j - 1][i] == "B" && board[j - 2][i] == "B" && board[j - 3][i] == "B"){
							return 2;
						}
					}
				}
	...
	public static int checkHorizontal(String[][] board){
			...
			for (int i = 0; i < board.length; i++){
				for (int j = 0; j < board[0].length; j++){
					...
					if (board[i][j] == "A"){
						// int startingRow = i;
						if(board[i][j + 1] == "A" && board[i][j + 2] == "A" && board[i][j + 3] == "A"){
							return 1;
						}
					}
				}
			...
			for (int i = 0; i < board.length; i++){
				for (int j = 0; j < board[0].length; j++){
					...
					if (board[i][j] == "B"){
						// int startingRow = i;
						if(board[i][j + 1] == "B" && board[i][j + 2] == "B" && board[i][j + 3] == "B"){
							return 2;
						}
					}
				}
